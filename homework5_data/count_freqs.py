    ...
    for l in corpus_file.readlines():
        if l:
            ...
            if line:
                ...
                ne_tag = fields[-1]
                word = " ".join(fields[:-1])
                counts[(word, ne_tag)] += 1
    return counts

def simple_conll_corpus_iterator(corpus_file, counts):
    """
...
class Hmm(object):
    ...
    def train(self, corpus_file, word_counts_dict):
        """
        Count n-gram frequencies and emission probabilities from a corpus file.
        """
        ngram_iterator = \
            get_ngrams(sentence_iterator(simple_conll_corpus_iterator(corpus_file, word_counts_dict)), self.n)
